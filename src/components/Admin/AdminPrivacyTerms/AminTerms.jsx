...
const parseTerms = (body) => {
  ...
  const points = parts.slice(1).map((item) => {
    ...
    return {
      heading: lines[0]?.replace(/^\d+\.\s*/, ""), // remove number
...
export default function AdminTerms() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (terms?.id) {
        res = await dispatch(updateTerms(terms.id, payload));
      } else {
        res = await dispatch(createTerms(payload));
      }
  ...
  return (
    <div className="admin-terms-div">
      <h5 className="admin-terms-title">Terms & Conditions</h5>
